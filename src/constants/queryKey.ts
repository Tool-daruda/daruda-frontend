export const MYPAGE_QUERY_KEY = Object.freeze({
  MY_INFO: (userId: string) => ['myInfo', userId], // 개인정보
  MY_POST_LIST: (userId: string, pageNo?: number) => ['myPostList', userId, pageNo], // 작성 글
  MY_FAVORITE_POST_LIST: (userId: string, pageNo?: number) => ['myFavoritePostList', userId, pageNo], // 관심 글
  MY_FAVORITE_TOOL_LIST: (userId: string) => ['myFavoriteToolList', userId], // 관심 툴
});
