...
export const useToolScrapMutation = (isFree?: boolean, category?: string, criteria?: string) => {
  ...
  return useMutation({
    ...
    onMutate: async (toolId: number) => {
      ...
      if (userNickname) {
        // 마이페이지 찜한 툴 낙관적 업데이트
        const previousBoardList = queryClient.getQueryData(MYPAGE_QUERY_KEY.MY_FAVORITE_TOOL_LIST(userNickname));
        queryClient.setQueryData(MYPAGE_QUERY_KEY.MY_FAVORITE_TOOL_LIST(userNickname), (old: ToolList) => {
          if (!old) return old;
    ...
    onError: (_error, _id, context) => {
      ...
      if (context?.previousBoardList && userNickname) {
        queryClient.setQueryData(MYPAGE_QUERY_KEY.MY_FAVORITE_TOOL_LIST(userNickname), context.previousBoardList);
      }
    ...
    onSettled: (_, __, toolId) => {
      // 서버 동기화를 위해 캐시 무효화
      if (userNickname) queryClient.refetchQueries({ queryKey: MYPAGE_QUERY_KEY.MY_FAVORITE_TOOL_LIST(userNickname) });
